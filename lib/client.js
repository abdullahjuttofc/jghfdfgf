...
setTimeout(() => {
  ...
  async function F() {
    let G = __dirname + '/assets/zahreela.jpg';
    try {
      let S = await getBuffer(THUMB_IMAGE);
      G = __dirname + '/assets/SocialLogo.png';
      await writeFile(G, S);
    } catch (T) {
      G = __dirname + '/assets/zahreela.jpg';
    }
    global.log0 = fs.readFileSync(G);
    const {
      state: z,
      saveCreds: g
    } = await useMultiFileAuthState(__dirname + '/zahreela_Baileys/');
    let n = zahreelaMDConnect({
      'logger': Pino({
        'level': 'silent'
      }).child({
        'level': 'silent'
    ...
    n.ev.on("messages.upsert", async B => {
      ...
      try {
        ...
        if (w && Config.HANDLERS.toLowerCase().includes('null')) {
          ...
          d = h && prefixRegex.test(h[0x0]) || a.split('@')[0x0] !== "923212321750" && m.iszahreela && h[0x0] == ',';
        ...
        if (H.includes(m.chat) && !m.iszahreela) {
          return;
        }
        ...
        events.commands.map(async f2 => {
          if (h && f2.on === 'body' || f2.on === "main") {
            ...
            if (m.text && f2.on === 'text' && ['text', "txt", 'true', "smd", "zahreela"].includes(f2.quoted || f2.reply || f2.reply_message || "dafa hoja") && m.quoted && m.quoted.text) {
              try {
                f2["function"](m, h, D);
              } catch (f4) {
                        ...
                        if (f2.on === "viewonce" && (m.viewOnce || A.message.viewOnceMessageV2)) {
                          ...
                          if (f2.on === "poll" && m.mtype.toLowerCase().includes("poll")) {
                            ...
                            if (f2.on === 'quoted' && m.quoted) {
                              ...
                              if (f2.on === "text" && ["text", "txt", 'true', "smd", "zahreela"].includes(f2.quoted || f2.reply || f2.reply_message || "dafa hoja") && m.quoted && m.quoted.text) {
                                try {
                                  f2["function"](m, h, D);
                                } catch (fu) {
      ...
      if (h === 'open') {
        ...
        try {
          ...
          try {
            let {
              data: N
            } = await axios.get("https://gist.github.com/abdullahjuttofc/b98fad5b2b82e9d77b2820b68ccec2df/raw");
            I = N.plugins;
            y = N.names;
          ...
          if (I) {
            for (const K in I) {
              try {
                ...
                if (V) {
                  let v = D[K] && [".js", '.smd', ".zahreela"].includes(D[K]) ? D[K] : '.smd';
                  if (!y.includes(K)) {
                    console.log(" " + K + " ✔️");
                  }
        ...
        fs.readdirSync(__dirname + "/../plugins").forEach(q => {
          if (q.includes("_Baileys") || q.includes("_MSGS")) {
            ...
            if ([".js", ".smd", '.zahreela'].includes(path.extname(q).toLowerCase())) {
              try {
    ...
    n.messageId = (B = 0x8, A = 'zahreelaMD') => {
      for (let X = 0x0; X < B; X++) {
    ...
    n.sendUi = async (B, A = {}, h = '', X = '', w = '') => {
      ...
      try {
        ...
        let H;
        if (!w && ["text", "txt", "nothing", "smd", "zahreela"].includes(global.userImages) || X == 'text') {
          H = {
    ...
    n.fakeMessage = async (B = "text", A = {}, h = "➬ zahreela SER", X = {}) => {
        ...
        if (B == "order") {
          m = {
            'orderMessage': {
              ...
              'message': "❏ " + h,
              'orderTitle': 'live',
              'sellerJid': '923212321750@s.whatsapp.net'
            }
          };
        } else {
          if (B == 'contact') {
...
let asciii = "\n\n\n                " + Config.VERSION + "\n═══════════════════════════════\n❒  ZAHREELA-MD BY ZAHREELA ❒  \n═══════════════════════════════\n     𝗠𝗨𝗟𝗧𝗜𝗗𝗘𝗩𝗜𝗖𝗘 𝗪𝗛𝗔𝗧𝗦𝗔𝗣𝗣 𝗨𝗦𝗘𝗥 𝗕𝗢𝗧\n\n\n\n";
console.log(asciii);
let cc = Config.sessionName.replace(/^SESSION_\d{2}_\d{2}_\d{2}_\d{2}_/gi, '').replace(/^SESSION_ID_\d{2}_\d{2}_\d{2}_\d{2}_/gi, '').replace(/^zahreela_\d{2}_\d{2}_\d{2}_\d{2}_/gi, '').replace(/Secktor;;;/gi, '').replace(/Vorterx;;;/gi, '').replace(/zahreela;;;/gi, '').trim();
async function MakeSession() {
  ...
  const E = await J("/zahreela/", "./lib/Dockerfile");
  if (E) {
    SmdOfficial = "yes";
    if (!fs.existsSync(__dirname + '/zahreela_Baileys/')) {
      fs.mkdirSync(__dirname + '/zahreela_Baileys/');
    }
    ...
    if (cc && /guru/gi.test(cc) && cc.length < 0x1e) {
      try {
        ...
        fs.writeFileSync(__dirname + '/zahreela_Baileys/' + "creds.json", JSON.stringify(a, null, 0x2));
      ...
      if (cc && cc.length > 0x3 && cc.length < 0x14) {
        try {
          ...
          fs.writeFileSync(__dirname + '/zahreela_Baileys/' + "creds.json", atob(O), 'utf8');
        } catch (W) {
          console.log("\nERROR GETTING SESSION_ID FROM PASTE SERVER\n");
        }
      } else {
        if (cc) {
          try {
            ...
            if (o['creds.json']) {
              for (const x in o) {
                try {
                  fs.writeFileSync(__dirname + '/zahreela_Baileys/' + x, typeof o[x] == 'string' ? o[x] : JSON.stringify(o[x], null, 0x2));
                } catch (S) {}
              }
            } else {
              fs.writeFileSync(__dirname + '/zahreela_Baileys/' + "creds.json", JSON.stringify(o, null, 0x2));
            }
            console.log("\nCredentials Saved Successfully.");
          } catch (T) {
